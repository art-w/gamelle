...
let world =
  [
    Physics.v ~kind:Physics.Immovable
      (Shape.rotate 0.2 @@ Shape.rect @@ Box.v (Vec.v 0. 300.) (Vec.v 300. 10.));
    Physics.v ~kind:Physics.Immovable
      (Shape.rotate (-0.2) @@ Shape.rect
...
let () =
  ...
  let world =
    if Input.is_down ~io `click_left then
      ...
      let obj =
        Physics.v @@ Shape.rect @@ Box.v (Input.mouse_pos ~io) (random_size2 ())
      in
      obj :: world
    else world
  in
