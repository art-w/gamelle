...
let init_ball () =
  Physics.add_velocity (V2.v (Random.float 30.0 -. 15.0) (-1_000.0))
  @@ Physics.make ~mass:1.0 ~restitution:1.0 ~kind:Movable
       (Shape.circle (P2.v 500.0 0.0) 50.0)
...
let initial_state =
  {
    player1 = init_player (P2.v 200.0 200.0);
    player2 = init_player (P2.v 800.0 200.0);
    ball = init_ball ();
    points1 = 0;
    points2 = 0;
  }
...
let world =
  [
    Physics.make ~restitution ~kind:Immovable
      (Shape.circle (P2.v 500.0 200.0) 10.0);
    Physics.make ~restitution:0.5 ~kind:Immovable
      (Shape.rect @@ Box2.v (P2.v 0.0 (-1500.0)) (V2.v 1000.0 1010.0));
    Physics.make ~restitution:0.5 ~kind:Immovable
      (Shape.rect @@ Box2.v (P2.v 490.0 200.0) (V2.v 20.0 1000.0));
    Physics.make ~restitution:0.5 ~kind:Immovable
      (Shape.rect @@ Box2.v (P2.v (-1000.0) (-1000.0)) (V2.v 1010.0 1500.0));
    Physics.make ~restitution:0.5 ~kind:Immovable
      (Shape.rect @@ Box2.v (P2.v 1000.0 (-1000.0)) (V2.v 1010.0 1500.0));
    Physics.make ~restitution:0.0 ~kind:Immovable
      (Shape.rect @@ Box2.v (P2.v (-1000.0) bottom) (V2.v 3500.0 1000.0));
  ]

let block_player1 =
  Physics.make ~restitution ~kind:Immovable
  @@ Shape.rect
  @@ Box2.v (P2.v 500.0 (-1000.0)) (V2.v 1000.0 2000.0)
...
let update_player ~io ~player:{ shape = player; jumps } ~gravity left right up
  ...
  let player =
    if Event.is_pressed ~io left then
      Physics.add_velocity (V2.v (-.horz_speed *. dt) 0.0) player
    else player
  in
  let player =
  ...
  let player =
    if Event.is_pressed ~io down then
      Physics.add_velocity (V2.v 0.0 (10_000.0 *. dt)) player
    else player
  in
  let jumps =
    if V2.y (Physics.center player) >= bottom -. player_radius -. 10.0 then 0
    else jumps
  in
  let player, jumps =
    if Event.is_down ~io up && jumps < 2 then
      (Physics.add_velocity (V2.v 0.0 (-60000.0 *. dt)) player, jumps + 1)
    else (player, jumps)
  in
...
let () =
  ...
  let io = View.translated (0.0, 500.0) io in
  ...
  else if V2.y (Physics.center ball) > 440.0 then
    if V2.x (Physics.center ball) < 500.0 then
