...
let cell_of_coord board x y =
  match (x, y) with
  | 0, 0 -> board.left_top
  | 0, 1 -> board.left_center
  | 0, 2 -> board.left_bottom
  | 1, 0 -> board.center_top
  | 1, 1 -> board.center_center
  | 1, 2 -> board.center_bottom
  | 2, 0 -> board.right_top
  | 2, 1 -> board.right_center
  | 2, 2 -> board.right_bottom
  | _ ->
      invalid_arg (Printf.sprintf "cell_of_coord: invalid coordinate %i %i" x y)

let update_cell_of_coord board new_cell x y =
  match (x, y) with
  | 0, 0 -> { board with left_top = new_cell }
  | 0, 1 -> { board with left_center = new_cell }
  | 0, 2 -> { board with left_bottom = new_cell }
  | 1, 0 -> { board with center_top = new_cell }
  | 1, 1 -> { board with center_center = new_cell }
  | 1, 2 -> { board with center_bottom = new_cell }
  | 2, 0 -> { board with right_top = new_cell }
  | 2, 1 -> { board with right_center = new_cell }
  | 2, 2 -> { board with right_bottom = new_cell }
  | _ ->
      invalid_arg
...
let victory board =
  ...
  | {
   left_center = Some p1;
   center_center = Some p2;
   right_center = Some p3;
   _;
  }
    when p1 = p2 && p2 = p3 ->
      Some p1
  | {
   left_bottom = Some p1;
   center_bottom = Some p2;
   right_bottom = Some p3;
   _;
  }
    when p1 = p2 && p2 = p3 ->
      Some p1
...
let draw_cell ~io cell p =
  match cell with
  | Some Circle -> draw ~io Assets.circle ~at:p
  | Some Cross -> draw ~io Assets.cross ~at:p
  | None -> ()
...
let rec loop ~io state =
  ...
  let state =
    ...
    | Some Circle ->
        print_endline "Circle won";
        exit 0
    | Some Cross ->
        print_endline "Cross won";
        exit 0
    | None ->
